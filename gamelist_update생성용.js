...
function gameList(system){
    ...
    rompath = dFile+"\\share\\roms\\"+system;
    system =system;
    누락목록[system]=[];
...
if(process.argv[2]) console.log('process.argv[2]',process.argv[2]);
if(process.argv[3]) console.log('process.argv[3]',process.argv[3]); // 값이 있으면 redirect 용 출력 (컬러 없음)
                ...
                try{
                    fs.readdirSync(`${rootpath}/${img.name}/share/roms`, {withFileTypes:true}).forEach(_system=>{
                        if(_system.isFile()){
                            ...
                            try{
                                system = _system.name;
//if(['amigacd32','amigacdtv','atomiswave','dreamcast','fmtowns','iptv','neogeocd','','ps2','psx','saturn','scummvm','segacd','windows','x68000'].indexOf(system)<0) return;

                                if(isSqlite) console.log('   ', system);
                                    ...
                                    if(err){
                                        ...
                                        try {
                                            result.gameList.game.forEach((obj,idx,data)=>{
                                                ...
                                                /**/
//if( ((['amigacd32','amigacdtv','atomiswave','dreamcast','fmtowns','iptv','neogeocd','','ps2','psx','saturn','segacd','x68000'].indexOf(system)>-1
//        && ['.gdi','.cue','.m3u','.lst'].indexOf(path.extname(obj.path[0]))>-1 )
if( ((['.gdi','.cue','.m3u','.lst','.game'].indexOf(path.extname(obj.path[0]))>-1 )
    || ['scummvm','windows'].indexOf(system)>-1)
    && obj.empty=='false'){
//console.log(    path.extname(obj.path[0]) );
//    console.log('UPDATE', img.name, system, obj.empty, obj.pathsize, obj.path[0]);
    var _path = obj.path[0].replace(/'/g,"''");
    if(obj.path[0] == './valve.game'){
        var _desc = obj.desc[0].replace(/\n/g,'\\n');
        console.log(`UPDATE gamelist SET empty='false', pathsize=${obj.pathsize}, desc='${obj.desc}'\tWHERE sdimage='${img.name}'\tAND system='${system}'\tAND path='${_path}';`);
    }else if(obj.path[0] == './Half-Life.game'){
        var _desc = obj.desc[0].replace(/\n/g,'\\n');
        console.log(`INSERT INTO gamelist(sdimage, system, empty, path, pathsize, name, desc, image, imagesize, developer, releasedate, genre, players) `
                   +`VALUES('${obj.sdimage}','${obj.system}','false','${_path}', '${obj.pathsize}', '${obj.name}', '${_desc}', '${obj.image}', '${obj.imagesize}', '${obj.developer}', '${obj.releasedate}', '${obj.genre}', '${obj.players}');`);
    }else if(obj.path[0] == './half-life_opposing-force.game'){
//        console.log(`DELETE FROM gamelist WHERE WHERE sdimage='${img.name}'\tAND system='${system}'\tAND path='./gearbox.game';`)
//        console.log(`INSERT INTO gamelist(sdimage, system, empty, path, pathsize, name, desc, image, imagesize, developer, releasedate, genre, players) `
//                   +`VALUES('${sdimage}','${system}','false','${path}', '${pathsize}', '${name}', '${desc}', '${image}', '${imagesize}', '${developer}', '${releasedate}', '${genre}', '${players}');`);
        var _desc = obj.desc[0].replace(/\n/g,'\\n');
        console.log(`UPDATE gamelist SET empty='false', path='${_path}', pathsize=${obj.pathsize}, desc='${_desc}'\tWHERE sdimage='${img.name}'\tAND system='${system}'\tAND path='./gearbox.game';`);
    }else {
        console.log(`UPDATE gamelist SET empty='false', pathsize=${obj.pathsize}\tWHERE sdimage='${img.name}'\tAND system='${system}'\tAND path='${_path}';`);
    }
} else return;
                                                if(isSqlite) {
                                                    /**/
                                            ...
                                            console.log('    XML fieldsGet ERR',`${rootpath}\\${img.name}\\share\\roms\\${_system.name}\\gamelist.xml`,e);
                                        }
                                    }
                                });
                            }catch(e){
                    ...
                    console.log(`301    ${rootpath}\\${img.name}`, e);
                }
            } // if(dFile=='fields'){
        } // if(img.name.toUpperCase().indexOf('BATOCERA')>=0){
    }); // fs.readdirSync(rootpath, {withFileTypes:true}).forEach(img=>{
...
function getSystemInfo(dFile){
    var total = 0;
    try{
        fs.readdirSync(dFile+"/share/roms").forEach(_filelist=>{
            system = _filelist;
