...
var
  ...
  mimeTypes = Object.create(null), // dict pattern (http://speakingjs.com/es5/ch17.html#dict_pattern)
  types, typec, type, exts, extc, i;

// Initialize the MIME types dictionary by parsing a specific comment in THIS file:
try {
  types = /\/\*!\s*mime\.types\s*:\s*((.|\r?\n)+)\s*\*\//i.exec(fs.readFileSync(__filename, 'utf-8'))[1].split(/\r?\n/);
  typec = types.length;

  while (typec--) {
    exts = types[typec].trim().split(/\s+/);
    if (exts.length === 0) { continue; }

    type = exts.shift();
    for (i = 0, extc = exts.length; i < extc; i++) {
      mimeTypes[exts[i]] = type;
    }
  }
} catch (err) {
  //log.error(err, 'Cannot load MIME types; any MIME identification request will default to application/octet-stream');
}

exports.type = function(extOrPath) {
  var extPos, theType;

  if (!extOrPath) { return undefined; }

  extPos = extOrPath.lastIndexOf('.');
  theType = extPos < 0 ?
            mimeTypes[extOrPath.trim().toLowerCase()] :
            mimeTypes[extOrPath.substring(extPos + 1).trim().toLowerCase()];

  return theType || mimeTypes['bin'] || 'application/octet-stream';
};

exports.contentType = function(extOrPath) {
  var mimeType = exports.type(extOrPath);
...
/*!mime.types:
 ...
 application/mxf                                                           mxf
 application/octet-stream                                                  bin bpk buffer deploy dist distz dmg dms dump elc iso lha lrf lzh pkg so
 application/oda                                                           oda
